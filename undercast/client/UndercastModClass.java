...
public class UndercastModClass {
    ...
    public void preInit(FMLPreInitializationEvent event) {
        ...
        File oldConfig = new File(newConfig.getParentFile().getAbsolutePath()
                + "/overcastNetwork-unofficialMod.cfg");
    ...
    public void init(FMLInitializationEvent event) {
        TickRegistry.registerTickHandler(new UndercastTickHandler(),
                Side.CLIENT); // Register a tick handler for custom gui
                              // rendering.
        NetworkRegistry.instance().registerChatListener(new ChatListener()); // Register
                                                                             // a
                                                                             // ChatListener
                                                                             // to
                                                                             // handle
        ...
        NetworkRegistry.instance().registerConnectionHandler(
                new UndercastConnectionHandler()); // Register a connection
                                                   // handler to know whenever
        FMLClientHandler.instance().getClient().guiAchievement = new UndercastGuiAchievement(
                FMLClientHandler.instance().getClient());
        LanguageRegistry.instance().addStringLocalization("undercast.gui",
                "Toggle Overcast Network mod gui");
        LanguageRegistry.instance().addStringLocalization(
                "undercast.inGameGui", "Change server");
        LanguageRegistry.instance().addStringLocalization(
                "undercast.fullBright", "Toggle fullbright");
        LanguageRegistry.instance().addStringLocalization("undercast.settings",
                "Show Undercast mod settings");
    ...
    public void onGameTick(Minecraft mc) {
        ...
        if (mc.currentScreen instanceof UndercastGuiGameOver
                && UndercastConfig.toggleTitleScreenButton) {
            ((UndercastGuiGameOver) mc.currentScreen)
                    .setTitleScreenButtonState(false);
        }
        ...
        boolean isInGameGuiEmpty = !this.mc.gameSettings.showDebugInfo
                && !this.mc.gameSettings.keyBindPlayerList.pressed;
        // if the gui is enabled display
        // if chat is open and config says yes then show gui
        if (isInGameGuiEmpty
                && UndercastData.guiShowing
                && (mc.inGameHasFocus || UndercastConfig.showGuiChat
                        && mc.currentScreen instanceof GuiChat)) {
            // show fps
            if (UndercastConfig.showFPS) {
                mc.fontRenderer
                        .drawStringWithShadow(fps, width, height, 0xffff);
                height += 8;
            }
        }
        ...
        // if chat is open and config says yes then show gui
        if (isInGameGuiEmpty
                && UndercastData.isPlayingOvercastNetwork()
                && UndercastData.guiShowing
                && (mc.inGameHasFocus || UndercastConfig.showGuiChat
                        && mc.currentScreen instanceof GuiChat)) {
            // Server display
            if (UndercastConfig.showServer) {
                mc.fontRenderer.drawStringWithShadow("Server: \u00A76"
                        + UndercastData.getServer(), width, height, 16777215);
                height += 8;
            }
            ...
            if (UndercastConfig.showTeam && !UndercastData.isLobby) {
                mc.fontRenderer.drawStringWithShadow(
                        "Team: " + UndercastData.getTeam(), width, height,
                        getTeamColors());
                height += 8;
            }
            ...
            if (UndercastConfig.showGSClass
                ...
                mc.fontRenderer.drawStringWithShadow("Class: "
                        + UndercastData.currentGSClass, width, height, 2446535);
                height += 8;
            }
            // Friend display:
            if (UndercastConfig.showFriends) {
                mc.fontRenderer.drawStringWithShadow("Friends Online: \u00A73"
                        + UndercastCustomMethods.getOnlineFriends(), width,
                        height, 16777215);
                height += 8;
            }
            ...
            if (UndercastConfig.showPlayingTime) {
                mc.fontRenderer.drawStringWithShadow(
                        UndercastCustomMethods.getPlayingTimeString(), width,
                        height, 16777215);
                height += 8;
            }
            // Match Time display:
            if (UndercastConfig.showMatchTime && !UndercastData.isLobby) {
                mc.fontRenderer.drawStringWithShadow(
                        UndercastCustomMethods.getMatchTimeString(), width,
                        height, 16777215);
                height += 8;
            }
            // Map fetcher:
            if (UndercastConfig.showMap && !UndercastData.isLobby) {
                if (UndercastData.getMap() != null) {
                    mc.fontRenderer.drawStringWithShadow("Current Map: \u00A7d"
                            + UndercastData.getMap(), width, height, 16777215);
                    height += 8;
                } else {
            ...
            if (UndercastConfig.showNextMap && !UndercastData.isLobby) {
                if (UndercastData.getNextMap() != null) {
                    mc.fontRenderer.drawStringWithShadow("Next Map: \u00A7d"
                            + UndercastData.getNextMap(), width, height,
                            16777215);
                    height += 8;
                } else {
                    mc.fontRenderer.drawStringWithShadow(
                            "Next Map: \u00A78Loading...", width, height,
                            16777215);
                    height += 8;
                }
            }
            ...
            if (UndercastConfig.showKD && !UndercastData.isLobby) {
                mc.fontRenderer.drawStringWithShadow("K/D: \u00A73"
                        + UndercastCustomMethods.getKD(), width, height,
                        16777215);
                height += 8;
            }
            // show KK Ratio
            if (UndercastConfig.showKK && !UndercastData.isLobby) {
                mc.fontRenderer.drawStringWithShadow("K/K: \u00A73"
                        + UndercastCustomMethods.getKK(), width, height,
                        16777215);
                height += 8;
            }
            ...
            if (UndercastConfig.showKills && !UndercastData.isLobby) {
                mc.fontRenderer.drawStringWithShadow(
                        UndercastCustomMethods.getKillDisplayString(), width,
                        height, 16777215);
                height += 8;
            }
            ...
            if (UndercastConfig.showDeaths && !UndercastData.isLobby) {
                mc.fontRenderer.drawStringWithShadow(
                        UndercastCustomMethods.getDeathDisplayString(), width,
                        height, 16777215);
                height += 8;
            }
            // Kill Streak display
            if (UndercastConfig.showStreak && !UndercastData.isLobby) {
                mc.fontRenderer.drawStringWithShadow(
                        "Current Killstreak: \u00A75"
                                + (int) UndercastData.getKillstreak()
                                + "\u00A7f/\u00A75"
                                + (int) UndercastData.getLargestKillstreak(),
                        width, height, 16777215);
                height += 8;
            }
            // Score display
            if (UndercastConfig.showScore && !UndercastData.isLobby
                    && UndercastData.score != 0) {
                mc.fontRenderer.drawStringWithShadow("Score: \u00A79"
                        + UndercastData.score, width, height, 16777215);
                height += 8;
            }
        }

        // if you not on obs turn it off
        if ((UndercastData.team != Teams.Observers && !UndercastData.isGameOver)
                || !UndercastData.isPlayingOvercastNetwork()) {
            brightActive = false;
        ...
        if (isInGameGuiEmpty
                ...
                && (mc.inGameHasFocus || UndercastConfig.showGuiChat
                        && mc.currentScreen instanceof GuiChat)) {
            if (brightActive
                    && UndercastConfig.fullBright
                    && (UndercastData.team == Teams.Observers || UndercastData.isGameOver)) {
                mc.fontRenderer.drawStringWithShadow("Full Bright: \u00A72ON",
                        width, height, 16777215);
                height += 8;
            } else {
                if (!brightActive
                        ...
                        && (UndercastData.team == Teams.Observers || UndercastData.isGameOver)) {
                    mc.fontRenderer.drawStringWithShadow(
                            "Full Bright: \u00A7cOFF", width, height, 16777215);
                    height += 8;
                }
            }
        }
