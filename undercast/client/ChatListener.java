...
public class ChatListener implements IChatListener {
    ...
    public Packet3Chat clientChat(NetHandler handler, Packet3Chat packet) {
        ...
        String messageWithOutJson = ChatMessageComponent.func_111078_c(
                packet.message).func_111068_a(true);
        ...
        try {
            ...
            if (!message.startsWith("<") && !message.startsWith("[Team]")
                    && !message.startsWith("(From ")
                    && !message.startsWith("(To ") && UndercastData.isOC) {
                addLineToChatLines(message);
                if (!(UCInstance.chatHandler.handleMessage(message,
                        UndercastModClass.getInstance().username, player,
                        messageWithOutJson))) {
                    packet.message = null;
                }
                if (UndercastConfig.showAchievements) {
                    UCInstance.achievementChatHandler.handleMessage(message,
                            UndercastModClass.getInstance().username, player,
                            messageWithOutJson);
                }
                if (UndercastConfig.parseMatchState) {
                    if (ServersCommandParser.handleChatMessage(message,
                            messageWithOutJson)
                            && (message.contains("Online: ") || message
                                    .contains("-------- Overcast Network Servers"))) {
                        packet.message = null;
                    }
                }
                ...
                if (FMLClientHandler.instance().isGUIOpen(
                        UndercastServerGUI.class)
                        && (message.contains("Online: ") || message
                                .contains("-------- Overcast Network Servers"))) {
                    packet.message = null;
                }
        ...
        packet.message = UndercastChatHandler.handleTip(messageWithOutJson) ? null
                : packet.message;
        return packet;
    }

    public void addLineToChatLines(String line) {
        System.arraycopy(UndercastModClass.lastChatLines, 0,
                UndercastModClass.lastChatLines, 1, 99);
